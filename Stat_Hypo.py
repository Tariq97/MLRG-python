...
def Stat_Hypo(slip,lx,lz,mech):
    ...
    if mech == 'dp':
        hypz=gampdf.gampdf(np.linspace(0,1,dim_z),7.364,0.072)
    elif mech == 'dc':
        hypz=wblpdf.wblpdf(np.linspace(0,1,dim_z),0.692,3.394)
    elif mech == 'ds':
        hypz=gampdf.gampdf(np.linspace(0,1,dim_z),12.658,0.034)
    elif mech == 'ss':
        hypz=wblpdf.wblpdf(np.linspace(0,1,dim_z),0.626,3.921)
    elif mech == 'al':
        hypz=wblpdf.wblpdf(np.linspace(0,1,dim_z),0.612,3.353)

    # --> along dip
    hypz=hypz.reshape(len(hypz),1)
        
    # --> along strike
    hypx=normpdf.normpdf(np.linspace(0,1,dim_x),0.5,0.23)
    ...
    if  mech == 'dp' or mech == 'dc' or mech == 'ds':
        hyp_mean=gampdf2.gampdf(tmp_slip,2.616,0.623)
    elif mech == 'ss':
        hyp_mean=gampdf2.gampdf(tmp_slip,1.928,0.868)
    elif mech == 'al':
        hyp_mean=gampdf2.gampdf(tmp_slip,2.210,0.748)
    ...
    mm,nn=np.shape(tmp_slip)
    if mech == 'dp' or mech == 'dc' or mech == 'ds':
        hyp_max=wwbl2.wblpdf(tmp_slip,0.454,1.874)
    elif  mech == 'ss':
        hyp_max=wwbl2.wblpdf(tmp_slip,0.446,1.551)
    elif  mech == 'al':
        hyp_max=wwbl2.wblpdf(tmp_slip,0.450,1.688)
